...
class Server {
  ...
  static #httpsPort = appEnvVariables.httpsPort || 5001;
  static #decoder = new StringDecoder('utf-8');
  static #httpsServerOptions = {
    key: fs.readFileSync('./https/key.pem'),
    cert: fs.readFileSync('./https/cert.pem'),
  };

  httpServer = http.createServer(function (req, res) {
    console.log('here comes an unsecured request');
    // redirect to HTTPS server when request comes on this server
    res.writeHead(301, {
      Location:
        'https://' + req.headers['host'].replace(Server.#httpPort, Server.#httpsPort) + req.url,
    });
    res.end();
  });
  ...
  httpsServer = https.createServer(Server.#httpsServerOptions, (req, res) => {
    this.requestHandler(req, res);
  });

  init() {
    this.httpServer.listen(Server.#httpPort, function () {
      console.log('\x1b[35m%s\x1b[0m', `HTTP Server listening on port ${Server.#httpPort}`);
    });
    this.httpsServer.listen(Server.#httpsPort, function () {
      console.log('\x1b[35m%s\x1b[0m', `HTTPS Server listening on port ${Server.#httpsPort}`);
    });
  }
}
...
//   init() {
//     const port = process.env.PORT ?? Server.#httpPort;
//     this.httpServer.listen(port, function () {
//       console.log('\x1b[35m%s\x1b[0m', `HTTP Server listening on port ${port}`);
//     });
//   }
// }
